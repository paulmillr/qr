...
The video or image feed is not always RGBA. If we ask for RGBA and the real stream
uses another format, the data is converted to RGBA first, and then we convert it
again to grayscale. That is wasted work.

QR decoding only needs luma (a grayscale image). Most common formats (NV12, I420, etc.)
already contain luma as a separate plane. If we use the original format, we can:

- skip the grayscale conversion completely (this saves `4 * pixels` of work),
- put less load on the source/decoder, because it does not need to convert to RGB.

Note: luma values can differ a little between formats (JPEG vs WebP vs NV12).
This is a small cost compared to the large amount of work we save.

One general problem: everything depends heavily on the binarizer. Because of this,
"making the test vectors pass" often turns into "retry with different binarizations".
This is the wrong approach. It is easy to find a set of retries that works for a
chosen set of photos and videos. But we do not control the camera, the image format,
or how many times the image was converted. So instead of retrying binarization,
the decoder must be robust, and it should retry the actual crosses.

## Pyramid

This is a basic technique, and almost every decoder uses it. We create N layers of
the image, each one smaller than the previous, and try to decode each layer.

This is also a kind of retry, but a cheap one, because small layers need much less
work. With x2 reduction, each layer costs about 25% of the layer above it
(4 pixels on the lower layer = 1 pixel on the higher layer). With 4 layers, the
total extra work is only a little more than 30%.

It also gives us a fast path: a big, clearly visible, low-version QR code can be
decoded from a small layer very quickly.

## Modes

There are two modes for QR decoding: video and photo.

### Video

This is the most important mode for the scanner, and the one we optimize for most.

The main difference: the time budget per frame is very tight, but the number of
retries is almost unlimited, because new frames keep coming. If we cannot decode
the current frame (blur, bad focus, shaking hands, sensor noise), we do not need to
retry or refine the frame or the finders. We can simply stop and wait for the next
frame. If we spend too much time retrying inside one frame, we may miss the next
frame, which could be much better.

This also means we should aim for zero memory allocations per frame.

For the scanner, we assume by default that the frame contains a QR code, and most
likely only one.

### Photo

Most libraries (ZXing, for example) are optimized for this mode.

Here there are no free retries: no new frame will come. So all retries must happen
on the single image we have. Also, nobody will move the camera to the next QR code,
so decoding several QR codes in one image should be an option.
